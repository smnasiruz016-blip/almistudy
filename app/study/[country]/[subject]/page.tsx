...
// Prebuilt: country x subject where universities exist (2,064) PLUS the origin
// guides served through this route as "from-<origin>" (88). Anything else 404s.
export function generateStaticParams() {
  return [...StaticIndex.countrySubjectPairs(), ...StaticIndex.originGuides()];
}
...
export async function generateMetadata({
  ...
  const description = unis
    ? `${unis.length.toLocaleString("en-US")} accredited ${unis.length === 1 ? "university" : "universities"} in ${c.name} teaching ${subjectName} — no agent, no commission. Each is verified against a recognized national accrediting body.`
